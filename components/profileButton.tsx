...
export default function ProfileButton() {
  ...
  */
  useEffect(() => {
    if (session && session.user?.name) {
      // Fetch user information from the database and update the state
      getUser(session.user.name).then((fetchedUser) => {
        setUser(fetchedUser);
        // Fetch the user's shop balance and update the state
        getUserShopBalance(fetchedUser.username).then((balance) => {
          setShopBalance(balance?.toString() ?? "0");
        });
      });
    }
  }, [session]);
  ...
  */
  const onSignUp = async () => {
    const user = await createUser(usernameInput, passwordInput);
    signIn("credentials", {
